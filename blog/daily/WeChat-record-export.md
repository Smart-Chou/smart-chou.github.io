...
比起我们这群嘴上教小龙哥做产品的人不一样，[hangcom](https://www.zhihu.com/people/hangcom) 则是用 WX Backup 微信聊天记录导出这个小工具，来让我们实现微信聊天记录的导出和保存。
...
在存放目录下，每个导出的聊天列表都会以「聊天对象\_微信ID」为命名的文件夹打包，并分类成语音（audio）、图片（image）、视频（video），以及一些网页文件（css、img、index.html 等）。文字的聊天记录则是存在 `js - message.js` 文件中。
...
其次，浏览页面的时间轴选项只精确到月份，因此当你要找当月中旬的聊天记录时，就会非常痛苦……尽管`Cmd ⌘ + F` 的网页搜索有一定帮助，但要精准找到某一条聊天记录的话，仍然要耗费不少时间。
